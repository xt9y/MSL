...
class Daemon {
    ...
    private func ensurePacmanKeyring() async {
        ...
        fflush(stdout)
        let cmd = "rm -f /var/lib/pacman/db.lck && chown -R root:root /root/.gnupg 2>/dev/null; chmod 700 /root/.gnupg 2>/dev/null; pacman-key --init && pacman -Sy --noconfirm archlinuxarm-keyring && pacman-key --populate archlinuxarm && pacman -Syy && touch \(marker)"
